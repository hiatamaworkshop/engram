...
```
L1 tags:    空間座標（Tagger がベクトル化 → 384次元位置 + 16bit フラグ付与）
L2 summary: ラベル（sense() で遠くから見える。ベクトル化されない）
...
// ExperienceCapsule の1ノードとして投入
{
  ...
  summary: `exp: ${dominantAxis}/${dominantState} ${durationMin}m v=${valenceBalance}`,
  payload: {
...
- **tags が空間位置を決める** — Tagger が tags を Sentence-BERT でベクトル化し、Sphere 空間上の座標を算出。同時に 16bit フラグをキーワードマッチで付与（decay 率、heat 倍率等の物理属性）。flags は `0` で投入し Tagger に委ねる
- **summary はラベル** — ベクトル化されない。エージェントが `sense()` した時に遠くから見えるヘッドライン。`exp: seeking/exploring 57m v=+0.3` のような簡潔な形式
