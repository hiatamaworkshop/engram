...
Experience Package は Sphere の ExperienceCapsule 作法に従って投入する。consumer は AI エージェント — 人間可読性は不要。Data Cost Protocol が適用される。

#### Sphere Node の L1-L4 と Experience Package の対応

```
L1 tags:    分類フラグ付与（Tagger → 16bit NodeFlag、regex マッチ）+ フィルタ検索
L2 summary: 意味的検索の唯一の軸（Parser → embedding → 384次元ベクトル = 空間座標）
L3 content: 本体（focus() で読まれる。Data Cost Protocol compact JSON）
L4 links:   生データ参照（engram nodeId 等）
```

#### Sphere Node 形式

```typescript
// ExperienceCapsule の1ノードとして投入
{
  tags: [
    "experience-package",
    "persona",
    ...techStack,      // "typescript", "docker"
    ...domain,         // "mcp-server", "receptor"
  ],
  summary: `experience: ${stateFlowNL}, ${sessionQualityNL}, ${techContext}`,
  // 例: "experience: frustrated exploration resolved through knowledge recall, positive flow session, typescript mcp receptor"
  payload: {
    content: JSON.stringify({
      $schema: "experience-package-v1",
      origin: { model, profileHash },
      label: { dominantState, stateFlow, valenceBalance, durationMs, meanIntensity },
      persona: { /* Persona object */ },
      priorBlock: [["H",...],["---"],["A",...],["---"],["F",...]]
    }),
    links: [...engramNodeIds],   // Prior Block 内の engram 参照
  },
  flags: 0   // Tagger に任せる
}
```
...
- **summary が空間座標を決定する** — Parser が summary を MiniLM (384dim) で embedding → Sphere 空間上の位置。tags は Tagger が regex マッチで 16bit NodeFlag に変換（ベクトル化ではない）
- **summary は自然言語で体験の質を記述** — MiniLM は自然言語の意味的類似度に最適化されている。数値（`confidence=0.45`）や略語（`exp: seeking/exploring 57m v=+0.3`）は embedding が弱い。stateFlow を人間語に翻訳し、技術ドメインを含める。action_log の embed target（例: `"typescript editing, stuck to exploring, switching"`）と同じ語彙圏に置くことで、future probe の centroid 検索で自然にヒットする
- **粒度の違いに注意** — action_log エントリはセッション内の瞬間（keypoint）、Experience Package はセッション全体の統合体。centroid（瞬間の重み付き平均）とは空間上の位置がずれる可能性がある。future probe での直接ヒットは副次的効果であり、主要アクセスは Facade 経由
- **Sphere Node 化は必須** — Facade は Sphere のフロントエンドであり、Facade `/lookup` は裏で Sphere `/sphere/explore` を叩く。Facade で扱うデータは Sphere Node でなければならない。「Facade 経由だが Sphere Node 化しない」は成立しない
- **Facade がショーケーシングの主軸** — Facade が profile hash + model + ドメイン + 感情パターン等の構造的フィルタで検索・ランキング。summary の embedding 精度はフォールバック的な検索軸であり、Facade のルーティング・フィルタリングが Experience Package 発見の主経路
- **content が Data Cost Protocol** — Prior Block は既に compact JSON array で設計されている。Persona も JSON。8KB 制限（Sphere Gatekeeper）に対し、Package 全体 ~3-4KB で余裕。自然言語で同内容を記述すれば 30KB 超 — Data Cost Protocol がなければ成立しない
- **Persona のみ取得も可能** — content を parse して `.persona` だけ取ればよい。Package は Persona の上位互換（エンベロープ）であり、破壊的変更ではなく拡張
- **現行 Persona も Sphere 配置済み** — `personaToCapsule()` で normalNode として配置。Facade `/lookup` で future probe のヒット対象。Experience Package はこの拡張
