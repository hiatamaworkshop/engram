...
#### 自動学習モード (learn mode) — 2026-03-23 実装

手動 `calibrate.ts` に加え、セッション体験から δ を漸進的に自動調整する opt-in モード。

```
engram_watch start                   → 通常モード。δ は読むだけ
engram_watch start (learn: true)     → 学習モード。セッション終了時に δ を微調整
```

**概念**: learned delta は受容感度の調整であり、トリガ結果の良し悪しの評価ではない。「発火後に良い結果だったか」ではなく「この軸は発火しすぎか、足りないか」を問う。AI 推論は一切使わない。

**ロジック** (`learn.ts`):

```
セッション終了時:
  1. session-points.jsonl から各軸の発火回数を集計
  2. 実発火頻度 = fires / session_hours
  3. 期待頻度 = 過去セッションの EMA (learn-ema.json)
  4. 偏差 = (actual - expected) / max(expected, 1.0)
  5. δ_new = clamp(δ_old - α × deviation, -0.30, +0.30)
  6. EMA 更新: ema_new = β × actual + (1 - β) × ema_old
```

| 定数 | 値 | 意味 |
|---|---|---|
| `ALPHA` | 0.03 | 学習率。1セッションで最大 ~0.03 の変化 |
| `EMA_BETA` | 0.3 | EMA 平滑化。直近セッション寄り |
| `DELTA_BOUND` | ±0.30 | δ の絶対上限（calibrate.ts と共通） |
| `MIN_SESSION_MS` | 300,000 | 5分未満のセッションは学習スキップ |

**安全装置**:
- opt-in のみ（`learn: true` を明示的に渡さない限り動かない）
- `DELTA_BOUND = ±0.30` でキャップ（calibrate.ts と同一制約）
- 保守的な学習率（α=0.03）で急激な変化を防止
- 短セッションガード（5分未満は学習しない）
- flow 軸は対象外（A gate invariant）

**calibrate.ts との関係**:
- `calibrate.ts` → 初期値設定・リセット（テストシナリオベース）
- `learn: true` → セッション体験からの漸進的調整
- `calibrate.ts` 再実行で learn の蓄積をリセット可能

**ファイル**:
- `receptor-learned.json` — δ 保存（calibrate.ts と learn.ts が共有）
- `receptor-output/learn-ema.json` — 期待頻度の EMA 状態（ランタイム成果物）
...
## アダプタ層設計 — マルチエージェント対応 (2026-03-23)

### 背景

現状の入力パイプラインは Claude Code 前提で動作する。
他の AI モデルやフレームワーク（Cursor、カスタムエージェント等）が receptor にイベントを送る場合、
ツール名・ペイロード構造・成功/失敗の表現がすべて異なるため、アダプタ層の切り出しが必要になる。

### 現状の問題点

1. **マッピングテーブルの粒度**: `CLAUDE_CODE_MAP` は `Record<string, NormalizedAction>` の 1:1 対応。
   他フレームワークでは 1 アクションが複数ツールに分散したり、1 ツールが複数の意味を持つことがある。
   単純なテーブルでは対応できない。

2. **result 判定のフレームワーク密結合**: `extractExitCode` の `returnCodeInterpretation` チェック、
   `extractSearchResultCount` の `numFiles` 抽出は Claude Code 固有。
   他フレームワークでは成功/失敗の表現が全く違う。

3. **RawHookEvent の曖昧さ**: `tool_name` / `tool_input` / `exit_code` という構造自体が Claude Code 前提。
   中間型として残すとフレームワークごとに歪んだ変換が発生する。

4. **HTTP 受信層の結合**: `parseHookPayload()` が `http.ts` 内にあり、
   Claude Code 以外の形式で `POST /receptor` に送信すると壊れる。

5. **ターンモデルの暗黙の前提**: Claude Code は user/agent ターンが明確だが、
   ストリーミング中のツール呼び出し、並列実行など、ターン境界が不明瞭なフレームワークがある。

### 設計方針

アダプタの出力契約を `NormalizedEvent` 一本に統一する。
`RawHookEvent` と `normalize()` はアダプタの内部実装に押し込む。

```
現状:
  生ペイロード → parseHookPayload() → RawHookEvent → normalize() → NormalizedEvent
                  (http.ts)                            (normalizer.ts)

目標:
  生ペイロード → adapter.translate() → NormalizedEvent
                  (adapter/*.ts)          ↓
                                    commander / heatmap / emotion（変更なし）
```
...
interface ReceptorAdapter {
  readonly name: string;           // "claude-code" | "cursor" | "custom"
  accept(payload: unknown): boolean;  // このペイロードを処理できるか
  translate(payload: unknown): NormalizedEvent | null;
}
```

- `accept()`: ペイロード構造からフレームワークを自動判別（`tool_name` の有無、特定フィールド等）
- `translate()`: フレームワーク固有の解析 + NormalizedAction マッピング + result 判定を一括で行う
- Claude Code アダプタは現在の `parseHookPayload + normalize` をそのまま内部に持つ

### HTTP 層の変更

```typescript
// http.ts requestHandler 内
const adapter = adapters.find(a => a.accept(json));
if (!adapter) { res.writeHead(400); res.end("unknown format"); return; }
const event = adapter.translate(json);
if (event) ingestEvent(event);  // ← ここから先は現状と同じ
```

代替: `POST /receptor?adapter=cursor` パラメータで明示指定も可。
自動判別とパラメータ指定の併用が現実的。

### 注意: ターン境界のオプショナル化

ターン境界 (`/turn`) を送れないフレームワークがある場合、
Commander はイベント間隔のみからターン数を推定するフォールバックが必要。
`turnInterval` は `NormalizedEvent` 上でオプショナルのまま維持する。

### 変更範囲

- **変更あり**: `http.ts`（アダプタ振り分け）、`normalizer.ts`（Claude Code アダプタとして再配置）
- **変更なし**: emotion, commander, heatmap, meta, passive, persona — core は一切触らない