...
> 2026-03-19 設計検討。実装は未着手。
> engram の代謝パラメータを状況適応型に進化させる構想。
...
### Phase 1: 密度ベース淘汰
- batch scan のついでに ingestedAt の min/max を取得
- span + nodeCount → density 算出
- density から decay rate を動的導出
- span から容量上限を動的導出
- 追加ファイル・追加クエリなし

### Phase 2: fixed soft demotion
- fixed ノードに半減期 60日の指数 decay（config で調整可能）
- recall hit で weight リセット
- weight < 1.0 で自動 recent 降格
- flag は即座の降格として維持

### Phase 3: 消去通知 → sink
- 消去されるノードの summary/tags を sink に emit
- receptor 経由でホットメモ対象
