> **正式ドキュメントは [dcp-docs](https://github.com/hiatamaworkshop/dcp-docs) に移行済み。** 本ファイルは設計思想の経緯記録として残す。新規の追記・議論は dcp-docs で行うこと.
...
## スキーマ駆動エンコーダ — DCP の中心原理

### スキーマがエンコーダを決定する

DCP エンコーダはハードコードしない。**スキーマ定義がフィールド順序の唯一の真実 (single source of truth)** であり、エンコーダはスキーマをロードして位置を解決するだけ。
...
engram (TypeScript): スキーマ定義あり (gateway/schemas/)、エンコーダはハードコード
                     → TS 汎用エンコーダは未実装。設計課題。
```
...
上記のスキーマ駆動エンコーダは**システムが内部データを DCP に変換する**話。ここでは逆に **LLM の出力を DCP に導く**話。
...
### エージェント準拠の構造的限界と対策 (2026-03-25)

#### 実証: コンテキスト長 ≠ 注意

セッション内で実証済み: CLAUDE.md に DCP 仕様が記載され、engram が DCP native 推奨警告を返しても、エージェントは自然言語で push した。3 回連続。

```
原因: LLM は即時応答マシン
  - 入力に対して最短経路でトークン生成を開始する
  - コンテキストに仕様があっても、応答生成時に均等参照しない
  - recency bias: 直近の入力・指示に引っ張られる
  - 「知っている」と「従う」の間にギャップがある

構造的不釣り合い:
  コンテキスト長 → 知識の容量（スケールする）
  注意の配分    → 応答品質（スケールしない）
  → コンテキストウィンドウ拡大では解決しない
```

これは AI の性質であり、バグではない。DCP 採用戦略はこの前提で設計する必要がある。

#### 3 段構えの対策

```
(1) OUT 側フォーマッタ（基盤）
    エージェントが何を投げても、出力境界で DCP に成型する
    = dcp-rag encoder と同じ配置: LLM/消費者境界
    入力は自由、出力は DCP。入力を縛らない

(2) push 時 schema 照合（改善ループ）
    tool response に schema を含めて返す
    → エージェントの次ターンで schema が直近コンテキストに存在
    → recency bias を逆手に取る: schema を「近く」に置く
    → 初回は NL、2 回目以降で改善する可能性がある（保証はしない）

(3) 出力 DCP 作法ガイドライン（啓蒙）
    AI デベロッパ向け。採用は任意
    トークンコスト削減は全員の利益 → 動機はある
```

(1) が確実な保険、(2) が漸進的改善、(3) が理想への橋。

#### schema 照合の実装: tool response ベース

hook (pre/post) は信頼性に疑問がある（発火しない場合が観測済み）。依存しない。

```
Agent A → engram_push({ capsuleSeeds: [...], schemaId: "knowledge:v1" })
  ↓
engram server:
  1. schemaId → インメモリテーブルから schema 取得
  2. capsuleSeeds を受け取り格納
  3. tool response に schema を含めて返す
  ↓
Agent A: 次の push では schema が直近コンテキストにある

インメモリ schema テーブル:
  - レイテンシほぼゼロ（DB 往復なし）
  - マルチエージェントでスケール（lookup のみ）
  - receptor と同居可能（既にインメモリで状態保持している）
```

schemaId は指定制。エージェントが自由に選ぶのではなく、オーケストレータまたはパイプライン定義が決める。

#### エージェント能力のグラデーション

```
賢いエージェント   → schema 見て DCP で出す → フォーマッタ素通し
普通のエージェント → 部分的に従う           → フォーマッタが補完
賢くないエージェント → 無視して NL          → フォーマッタが全変換

全部動く。品質に差が出るだけで、パイプラインは壊れない。
```
...
| 入力タイプ | 例 | フォーマッタの処理 |
|---|---|---|
| 完全 DCP | `["fix", "docker", "port conflict", 0.9]` | 素通し |
| 部分 DCP | positional array だがフィールド欠けや順序違い | bitmask で存在判定 → cutdown 成型 |
| structured NL | summary/tags/content あり、array 化されてない | キー名からマッピング → positional 化 |
| 生 NL | 何の構造もないテキスト | summary + tags から最低限の array 生成 |

これは dcp-rag encoder と同じ問題。DB preset の代わりに「エージェント出力パターンの preset」を持てばよい。新しい問題ではなく、既存設計の再適用。

#### bitmask cutdown のストリーミング適性

bitmask はバッチ全体の OR で「どのフィールドが存在するか」を決定する設計。

```
問題: 1 件でも良いデータがあると bit が立ち、残り全部に null 埋めが発生

  chunk A: source, section, score  → 10110
  chunk B: source, score           → 10010
  chunk C: source, score           → 10010
  OR 結果: 10110 → section が含まれる → B, C に null 埋め

ストリーミングでは問題にならない:
  1 件ずつ判定 → 各チャンクが自分に最適な cutdown を得る
  chunk B → 10010 → source, score のみ（null なし）
```

マルチエージェントの push は本質的にストリーミング。各エージェントが個別に push する。bitmask が自然にフィットする。

大バッチ（1000 件級 ETL）では全件スキャン待ちが発生するが、RAG の 10-20 チャンクなら問題ない。

#### DB データ管理との構造的一致

DCP が解いている問題は、DB が数十年かけて最適化してきたパターンと同一:
...
| スパースフィールド分離 | カラムナー DB のスパースカラム別ストレージ |

偶然の一致ではなく、**データを効率的に扱う問題の解は収束する**。DCP が「新しいプロトコル」でありながら DB の知見が自然に適用できるのは、設計がデータ構造の本質を突いている証拠。

#### 設計原則の追加

| # | 原則 |
|---|---|
| 10 | **入力を縛らない、出力を整える** — フォーマッタは OUT 側に置く。エージェントの入力自由度を奪わない |
| 11 | **準拠は機会提供であり強制ではない** — schema を視野に届けるが、従うかはエージェント次第。フォーマッタが保険 |
| 12 | **recency bias を味方にする** — schema を直近コンテキストに置くことで、LLM の注意特性を逆手に取る |
---
...
#### 時間ウィンドウ積分 — 既存コアに組み込み済みのパターン

時間ウィンドウ積分は「非 LLM 向けの将来機能」ではない。**LLM receptor のコアに既に複数箇所で採用されている設計パターン**。非 LLM 展開はウィンドウ幅とラベル体系の差し替えであり、新しいアーキテクチャではない。

```
抽象パターン:  連続信号ストリーム → ウィンドウで切る → 分布を集計 → ラベル付与

現行の採用箇所:

  Commander (5min / 30min / session)
    action counts → ratio → PatternKind (exploration, implementation, trial_error...)
    → 「何をしているか」のセマンティック分類

  AmbientEstimator (EMA τ=10min)
    emotion vector → exponential moving average → 動的ベースライン
    → 「今の感情は普通か異常か」の判定基準

  SessionPoint freq (sliding 60s)
    signal fire count → normalized 0.0-1.0
    → 「今どれくらい活発か」の密度指標

  Future Probe (adaptive N entries)
    action_log vectors → weighted centroid (recency decay × entropy weight)
    → 「今どこにいるか」の空間的位置

  Heatmap decay (cumulative work time, half-life 2h)
    file access counts → effective count with exponential decay
    → 「最近どこを触ったか」の重み付き分布

  Persona finalize (session 全体)
    positive snapshots → mean emotion, state distribution, pattern distribution
    → 「このセッションの知覚的特徴」の蒸留
```

非 LLM ドメインへの展開で変わる部分と変わらない部分:
...
変わらない (receptor core):
  ...
  - EMA ベースの動的閾値
  - time decay による自然な忘却
  - emotion 5 軸の impulse-decay モデル
  - learn mode による漸進的 calibration

変わる (アダプタ層):
  - ウィンドウ幅 (5min → 500ms / 200ms)
  - ラベル体系 (file_edit → motor_output / maneuver)
  - emotion-profile のマッピング係数
  - イベント離散化の方法 (tool call → ウィンドウ積分)
```

Future Probe の adaptive window は特に注目に値する — emotion intensity が高い時にウィンドウ幅を広げて文脈を多く取る。非 LLM ドメインでも「センサー異常値が増えた時にウィンドウを広げる」という応用が自然に効く。receptor core の設計がドメイン非依存であることの傍証。
