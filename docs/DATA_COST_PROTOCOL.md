...
### スキーマの概念進化 — 静的契約から動的インターフェースへ (2026-03-25)

従来のスキーマは **静的な契約書** だった。

```
JSON Schema       → バリデーション。人間が書いて人間が読む。
Protocol Buffers  → シリアライズ。コンパイル時に確定。
OpenAPI           → ドキュメント。エンドポイントの仕様書。
DB Schema         → テーブル定義。マイグレーションで管理。

共通点: 書いたら動かない。参照されるが、対話しない。コスト構造を持たない。
```

DCP のスキーマは **動的なインターフェース** である。

```
定義する     → gateway/schemas/ に SSOT。1 箇所だけ。
教育する     → tool description にインライン、エラーに定義含む。
参照する     → abbreviated $S でコンテキストに常駐 (~5 tokens)。
展開する     → GET /schemas/:id で必要時にフル取得。
検証する     → push 時に準拠チェック（warn, not reject）。
コスト制御   → 準拠 = abbreviated。非準拠 = expanded。コスト差が自然な誘導。
```

この進化が起きた理由は **トークンコスト** という制約の存在。

```
従来: スキーマのサイズは問題にならない（人間が読む限り、数百行の JSON Schema も許容される）
DCP:  スキーマを毎回全文送るとトークンを食う
      → abbreviated にしよう
      → 展開は必要時だけ
      → スキーマ自体がコスト構造を持つオブジェクトになった
```

スキーマが「定義」から「コスト構造を持つ動的な通信参加者」に変わった瞬間:

- **abbreviated** — スキーマの存在だけを ~5 tokens で伝える。TLS session ticket。
- **expanded** — フィールド + 型情報を ~30 tokens で伝える。教育目的。
- **full** — 定義全体 + 例示を返す。初回ハンドシェイクまたは `$S?` 要求時のみ。

スキーマ自体が密度のスペクトラムを持ち、状況に応じて最適な密度で出現する。これは従来のスキーマ概念には存在しなかった性質。

**制約が効率を生む。** phi-agent パターンの帰結として、トークンコストという制約がスキーマの概念を進化させた。
